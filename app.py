...
from flask import Flask, flash, render_template,redirect,url_for, request , jsonify, json
import sqlite3
...
def display():
    ...
    if request.method=="POST":
        ...
        try:  
            with sqlite3.connect("empass.db") as con:
                print("Connected")    
                cur = con.cursor()  
                print("Got cursor")  
...
def updatedisplay():
    ...
    if Emp_Name == 'Emp_Name':
        cur.execute("UPDATE employee SET Emp_Name = %s WHERE id = %s ", (Emp_Name, Emp_Name))
    elif Band == 'Band':
        cur.execute("UPDATE employee SET Band = %s WHERE id = %s ", (Band, Band))
    elif CCName == 'CCName':
        cur.execute("UPDATE employee SET CCName = %s WHERE id = %s ", (CCName, CCName))
    elif CCID == 'CCID':
        cur.execute("UPDATE employee SET CCID = %s WHERE id = %s ", (CCID, CCID))
